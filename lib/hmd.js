...
function sgmts(dbi, aid, vid, dirs) {
  return [
    sgmt(dbi, aid, vid,   '', SITE, HTI),
    sgmt(dbi, aid, vid,   '', MENU, HTI),
    sgmt(dbi, aid, vid, dirs, SECT, HTI)
  ];
}
exports.sgmts = sgmts;

function getHtm(dbi, aid, vid, dirs, base) {
  return new Promise((resolve, reject) => {
    db.fetch(dbi, aid, vid, dirs, base, 'htm')
    .then((cxt) => {
      var ps = sgmts(dbi, aid, vid, dirs);
      ps.unshift(meta(dbi, aid, vid, dirs, base));
      Promise.all(ps)
      .then(a => {
        cxt.meta = a[0];
        cxt.site = a[1];
...
exports.getHtm = getHtm;

function getHmd(dbi, aid, vid, dirs, base) {
  return new Promise((resolve, reject) => {
