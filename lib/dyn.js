...
router.get(config.INTDTA_HTM, (req, res) => {
  console.log("!!!HTM!!%s!", req.dom.db);
  htm.get(req.dom, req.dirs, req.base)
  .then (dta => {
    console.log('dyn:htm=%s!', JSON.stringify(dta));
    return hog.respond(req, res, 200, dta)
  })
  .catch(err => {
    console.log('PNF dyn:htm!');
    return reply.pnf(req, res, err, req.path);
  });
});
