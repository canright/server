...
function index(dom, pat) {
  console.log('FILE.INDEX: ' + path.join(config.MOUNT, dom.aid, dom.vid, pat));
  return path.join(config.MOUNT, dom.aid, dom.vid, pat);
}
exports.index = index;
...
exports.send = (req, res) => {
  ...
  return sendFile(req, res, index(req.dom, req.path));
}
...
function readFile(full) {
  console.log('file.readFile '+ full + '!');
  return new Promise((resolve, reject) => {
    console.log('fs.readFile: ' + path.join(global.appRoot, full));
    fs.readFile(path.join(global.appRoot, full), "utf8", (err, blob) => {
      if (!err)
        resolve(blob);
      else {
        console.log('readFile(%s) rejected', full);
        reject(err);
      }
    });
  });
...
exports.fetch = (req, res, pat) => {
  ...
  return readFile(index(req.dom, pat));
}
